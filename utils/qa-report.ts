...
export function renderQaReportMarkdown(report: QaRunReport): string {
  ...
  if (report.retriedTests.length) {
    lines.push(
      ...
      '',
      '| Total | Maximum attempts | Retry attempts | Average duration | Maximum duration | Total duration |',
      '| ---: | ---: | ---: | ---: | ---: | ---: |',
      `| ${report.retriedTestSummary.total} | ${report.retriedTestSummary.maximumAttempts} | ${report.retriedTestSummary.retryAttempts} | ${formatDuration(report.retriedTestSummary.averageDurationMs)} | ${formatDuration(report.retriedTestSummary.maximumDurationMs)} | ${formatDuration(report.retriedTestSummary.totalDurationMs)} |`,
      '',
      '## Retried Tests',
      '',
