...
export function renderQaReportMarkdown(report: QaRunReport): string {
  ...
  if (report.durationBudgetBreaches.length) {
    lines.push(
      ...
      '',
      '| Executed | Within budget | Within budget rate | Breaches | Breach rate | Threshold | Minimum duration | Average duration | Maximum duration | Total duration | Minimum over budget | Minimum over budget rate | Average over budget | Average over budget rate | Maximum over budget | Maximum over budget rate | Total over budget |',
      '| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |',
      `| ${report.durationBudgetBreachSummary.executed} | ${report.durationBudgetBreachSummary.withinBudget} | ${report.durationBudgetBreachSummary.withinBudgetRate}% | ${report.durationBudgetBreachSummary.total} | ${report.durationBudgetBreachSummary.breachRate}% | ${formatDuration(report.durationBudgetBreachSummary.thresholdMs || 0)} | ${formatDuration(report.durationBudgetBreachSummary.minimumDurationMs)} | ${formatDuration(report.durationBudgetBreachSummary.averageDurationMs)} | ${formatDuration(report.durationBudgetBreachSummary.maximumDurationMs)} | ${formatDuration(report.durationBudgetBreachSummary.totalDurationMs)} | ${formatDuration(report.durationBudgetBreachSummary.minimumOverBudgetMs)} | ${report.durationBudgetBreachSummary.minimumOverBudgetRate}% | ${formatDuration(report.durationBudgetBreachSummary.averageOverBudgetMs)} | ${report.durationBudgetBreachSummary.averageOverBudgetRate}% | ${formatDuration(report.durationBudgetBreachSummary.maximumOverBudgetMs)} | ${report.durationBudgetBreachSummary.maximumOverBudgetRate}% | ${formatDuration(report.durationBudgetBreachSummary.totalOverBudgetMs)} |`,
      '',
      '## Duration Budget Breaches',
      '',
      '| Test | Suite | Status | Duration | Over budget | Over budget rate |',
      '| --- | --- | --- | ---: | ---: | ---: |',
      ...report.durationBudgetBreaches.map((test) => (
        `| ${escapeTable(test.title)} | ${escapeTable(test.suite)} | ${test.status} | ${formatDuration(test.durationMs)} | ${formatDuration(test.overBudgetMs)} | ${test.overBudgetRate}% |`
      )),
    );
  }
