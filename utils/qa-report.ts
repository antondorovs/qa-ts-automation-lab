...
export function renderQaReportMarkdown(report: QaRunReport): string {
  ...
  const lines = [
    ...
    '',
    '| Executed | Total | Average | Median | P95 | Maximum |',
    '| ---: | ---: | ---: | ---: | ---: | ---: |',
    `| ${report.durationProfile.executed} | ${formatDuration(report.durationProfile.totalDurationMs)} | ${formatDuration(report.durationProfile.averageDurationMs)} | ${formatDuration(report.durationProfile.medianDurationMs)} | ${formatDuration(report.durationProfile.p95DurationMs)} | ${formatDuration(report.durationProfile.maximumDurationMs)} |`,
    '',
    '## Quality Gate Policy',
    '',
    '| Policy | Value |',
