...
test.describe('@utils @contract QA run intelligence', () => {
  ...
  test('duration budget breaches should identify executed tests over the configured maximum', () => {
    ...
    expect(markdown).toContain('| Executed | Within budget | Within budget rate | Breaches | Breach rate | Threshold | Minimum duration | Average duration | Maximum duration | Total duration | Minimum over budget | Average over budget | Maximum over budget | Maximum over budget rate | Total over budget |');
    expect(markdown).toContain('| 3 | 1 | 33.33% | 2 | 66.67% | 1.00s | 1.20s | 1.35s | 1.50s | 2.70s | 200ms | 350ms | 500ms | 50% | 700ms |');
