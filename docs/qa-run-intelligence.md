...
Failure details include a failure summary with total failed results, status split, average failed-test duration, and accumulated failed-test duration before the error messages.
...
When slow tests are present, the report includes a slow-test summary with the total count, configured threshold, minimum, average, and maximum duration before the detailed slow-test list.
